...
- **THEN** the system SHALL return HTTP 429 with path-appropriate error format and `rate_limit_error` type
...
- **THEN** the proxy SHALL return HTTP 403 with path-appropriate error format and `permission_error` type, message "Access denied"

#### Scenario: Non-blocked user-agent passes through
- **WHEN** a proxy request arrives with a UA not in `blocked_user_agents`
