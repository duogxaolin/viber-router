...
### Requirement: Usage data excludes server information
The system SHALL aggregate token usage by model only. The response MUST NOT include `server_id`, `server_name`, or any information that identifies upstream providers.

#### Scenario: Usage response shape
- **WHEN** a valid sub-key has usage data
- **THEN** each usage entry contains only: `model`, `total_input_tokens`, `total_output_tokens`, `total_cache_creation_tokens`, `total_cache_read_tokens`, `request_count`, `cost_usd`
...
#### Scenario: Active hourly_reset subscription
